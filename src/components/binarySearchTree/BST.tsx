...
class BST extends Component<IProps, IState> {
  ...
  updateView = (newMain: number, bst: BinarySearchTree = this.state.bst) => {
    const newBST = bst
  ...
  render() {
    return (
      <div className="pageContainer">
        <h2 id="sectionHeaderBST" className="sectionHeader">
          Binary Search Tree
        </h2>
        ...
        <div className="bstContainer">
          <p id="parentDisplay" className="bstLabel">
            {this.state.displayNode.parent > -1
              ? this.state.displayNode.parent
              : "Empty"}
          </p>
        </div>
        <div className="bstContainer">
          <p id="mainDisplay" className="bstLabel">
            {this.state.displayNode.main > -1
              ? this.state.displayNode.main
              : "Empty"}
          </p>
        </div>
        <div className="bstContainer">
          <p id="leftDisplay" className="bstLabel">
            {this.state.displayNode.left > -1
              ? this.state.displayNode.left
              : "Empty"}
          </p>
          <p id="rightDisplay" className="bstLabel">
            {this.state.displayNode.right > -1
              ? this.state.displayNode.right
              : "Empty"}
          </p>
        </div>
        <div className="bstContainer">
          <Button
            disabled={this.state.displayNode.left === -1 || this.state.loading}
            className="directionButton btn btn-info"
            id="leftButton"
          ...
          <Button
            disabled={this.state.displayNode.right === -1 || this.state.loading}
            className="directionButton btn btn-info"
            id="rightButton"
        ...
        <div className="descriptionContainer">
          <p id="description" className="description">
            This page allow items to be added to a binary search tree. A small
            section of the tree is displayed with buttons for navigation. The
            parent node is shown on top, the current value in the middle, and
