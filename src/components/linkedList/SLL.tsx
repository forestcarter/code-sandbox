    ...
    return (
      <div>
        ...
        <div className="flexRow">
          <Button
            disabled={this.state.loading || this.state.linkedList.length === 0}
            ...
            {arrayToRender.map(node => {
              return (
                <div className="listItem" key={node.nodeId}>
                  <ImageIcon fileName={`${node.nodeId}.png`} />

                  <div className="filmLabelContainer">
                    <div className="filmLabel">{node.filmName}</div>
                  </div>
                </div>
              )
            })}
          ...
          <Button
            disabled={this.state.loading || this.state.linkedList.length >= 7}
        ...
        <div className="descriptionContainer">
          <p className="description">
            ...
            <a href="https://swapi.co/"> SWAPI </a>using GraphQL. Data is stored
            in a singly-linked list with{" "}
            <button
              disabled={this.state.loading || this.state.linkedList.length >= 7}
              onClick={this.addElement}
              className="link"
            ...
            <button
              disabled={
                this.state.loading || this.state.linkedList.length === 0
              }
              className="link"
              onClick={this.subtractElement}
            >
